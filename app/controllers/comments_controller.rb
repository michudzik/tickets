  ...
  def create
    ...
    respond_to do |format|
      if @comment.save
        ...
        a = @comment.ticket.comments.pluck(:user_id).reject { |id| id == current_user.id }
        UserNotifier.notify_users(@comment.ticket.comments.pluck(:user_id).reject { |id| id == current_user.id }, @comment.ticket)
